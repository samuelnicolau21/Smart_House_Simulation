    ...
    while True:
        ...
        if mensagem_json.get("comando") == "descobrir":
            resposta_json = {
                ...
                "id": AC_ID,
                "status": "pronto",
                "endereco": [f"{AC_IP}", f"{AC_PORT}"],
                "funcionalidades": [
                    {"nome": "ligar/desligar", "parametros": []},
                    {"nome": "temperatura", "parametros": [{"nome": "valor", "tipo": "inteiro"}]},
                    {"nome": "modo", "parametros": [{"nome": "modo de operação", "tipo": "resfriar,aquecer,ventilar"}]}
                ]
            }
